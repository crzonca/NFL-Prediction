...
import Projects.nfl.NFL_Prediction.PlayoffHelper2019 as Playoffs
import Projects.nfl.NFL_Prediction.PlottingHelper2019 as Plotter
import Projects.nfl.NFL_Prediction.StandingsHelper2019 as Standings
...
def season():
    ...
    Standings.print_schedule_difficulty(nfl_teams,
                                        remaining=True,
                                        completed=regular_season_complete)
...
def week_14(teams, week_end_date, week=14):
    ...
    teams = set_game_outcome(teams, week, -1,
                             'Rams', 28, 4, 293, 22, 32, 2, 2, 455,
                             'Seahawks', 12, 0, 202, 22, 36, 0, 1, 308)

    teams = set_game_outcome(teams, week, -9,
                             'Eagles', 23, 3, 300, 33, 50, 2, 0, 418,
                             'Giants', 17, 2, 182, 15, 30, 2, 0, 255)
...
def week_15(teams, week_end_date, week=15):
    ...
    teams = set_game_outcome(teams,
                             ...
                             home_score=42,
                             home_touchdowns=6,
                             home_net_pass_yards=212,
                             home_pass_completions=15,
                             home_pass_attempts=24,
                             home_pass_tds=5,
                             home_interceptions_thrown=0,
                             home_total_yards=430,
                             away_name='Jets',
                             away_score=21,
                             away_touchdowns=2,
                             away_net_pass_yards=207,
                             away_pass_completions=18,
                             away_pass_attempts=32,
                             away_pass_tds=2,
                             away_interceptions_thrown=1,
                             away_total_yards=310)

    teams = set_game_outcome(teams, week, 6,
                             'Panthers', 24, 3, 269, 25, 42, 1, 3, 414,
                             'Seahawks', 30, 4, 274, 20, 27, 2, 1, 428)

    teams = set_game_outcome(teams, week, 6.5,
                             'Redskins', 27, 3, 251, 19, 28, 2, 0, 352,
                             'Eagles', 37, 4, 258, 30, 43, 3, 0, 415)

    teams = set_game_outcome(teams, week, -3,
                             'Titans', 21, 3, 269, 22, 37, 2, 1, 432,
                             'Texans', 24, 3, 234, 19, 27, 2, 2, 374)

    teams = set_game_outcome(teams, week, -3.5,
                             'Giants', 36, 5, 274, 21, 29, 2, 3, 412,
                             'Dolphins', 20, 2, 262, 23, 41, 2, 0, 384)

    teams = set_game_outcome(teams, week, -9,
                             'Chiefs', 23, 2, 327, 27, 34, 2, 1, 419,
                             'Broncos', 3, 0, 199, 18, 40, 0, 1, 251)

    teams = set_game_outcome(teams, week, 10.5,
                             'Bengals', 13, 1, 151, 17, 31, 1, 4, 315,
                             'Patriots', 34, 3, 116, 15, 29, 2, 0, 291)

    teams = set_game_outcome(teams, week, 6,
                             'Lions', 17, 2, 250, 24, 43, 0, 2, 295,
                             'Buccaneers', 38, 4, 446, 28, 42, 4, 1, 495)

    teams = set_game_outcome(teams, week, -4.5,
                             'Packers', 21, 3, 192, 16, 33, 1, 0, 292,
                             'Bears', 13, 1, 319, 29, 53, 1, 2, 415)

    teams = set_game_outcome(teams, week, -7,
                             'Raiders', 16, 1, 236, 22, 36, 1, 0, 364,
                             'Jaguars', 20, 2, 185, 17, 29, 2, 0, 262)

    teams = set_game_outcome(teams, week, 3,
                             'Cardinals', 38, 5, 219, 19, 25, 1, 1, 445,
                             'Browns', 24, 3, 241, 30, 43, 2, 1, 393)

    teams = set_game_outcome(teams, week, -10,
                             '49ers', 22, 2, 193, 22, 34, 1, 0, 313,
                             'Falcons', 29, 3, 201, 25, 39, 2, 0, 290)

    teams = set_game_outcome(teams, week, 1,
                             'Cowboys', 44, 5, 212, 15, 23, 2, 0, 476,
                             'Rams', 21, 3, 267, 34, 52, 2, 1, 389)

    teams = set_game_outcome(teams, week, 1,
                             'Chargers', 10, 1, 283, 28, 39, 1, 3, 345,
                             'Vikings', 39, 3, 207, 19, 25, 1, 1, 344)

    teams = set_game_outcome(teams, week, -1,
                             'Steelers', 10, 1, 178, 23, 38, 1, 4, 229,
                             'Bills', 17, 2, 131, 13, 25, 1, 1, 261)

    teams = set_game_outcome(teams, week, -8.5,
                             'Saints', 34, 4, 307, 29, 30, 4, 0, 424,
                             'Colts', 7, 1, 159, 18, 34, 0, 0, 205)
...
def week_16(teams, week_end_date, week=16):
    ...
    teams = set_game_outcome(teams, week, -7.5,
                             'Chargers', 17, 2, 265, 27, 39, 0, 0, 284,
                             'Raiders', 24, 3, 267, 26, 30, 1, 0, 366)

    teams = set_game_outcome(teams, week, 1.5,
                             'Redskins', 35, 5, 281, 29, 38, 3, 0, 361,
                             'Giants', 41, 6, 346, 28, 42, 5, 0, 552)

    teams = set_game_outcome(teams, week, 3.5,
                             'Titans', 28, 4, 248, 17, 27, 3, 0, 397,
                             'Saints', 38, 5, 275, 28, 40, 3, 0, 377)

    teams = set_game_outcome(teams, week, 3,
                             'Jets', 16, 1, 174, 16, 26, 1, 0, 259,
                             'Steelers', 10, 1, 185, 25, 37, 1, 2, 260)

    teams = set_game_outcome(teams, week, -6.5,
                             'Patriots', 24, 2, 271, 26, 33, 1, 0, 414,
                             'Bills', 17, 2, 176, 13, 26, 2, 0, 268)

    teams = set_game_outcome(teams, week, -7,
                             '49ers', 34, 3, 215, 16, 27, 1, 2, 334,
                             'Rams', 31, 4, 323, 27, 46, 2, 1, 395)

    teams = set_game_outcome(teams, week, -8,
                             'Broncos', 27, 3, 198, 26, 34, 1, 0, 348,
                             'Lions', 17, 1, 95, 12, 24, 1, 0, 191)

    teams = set_game_outcome(teams, week, -7.5,
                             'Falcons', 24, 3, 383, 32, 45, 1, 2, 518,
                             'Jaguars', 12, 1, 173, 14, 32, 1, 0, 288)

    teams = set_game_outcome(teams, week, 10,
                             'Browns', 15, 2, 184, 20, 33, 2, 1, 241,
                             'Ravens', 31, 4, 238, 20, 31, 3, 0, 481)

    teams = set_game_outcome(teams, week, -7,
                             'Colts', 38, 3, 106, 14, 27, 0, 0, 324,
                             'Panthers', 6, 0, 199, 28, 45, 0, 3, 286)

    teams = set_game_outcome(teams, week, 2,
                             'Dolphins', 38, 5, 406, 31, 52, 4, 1, 502,
                             'Bengals', 35, 4, 371, 33, 56, 4, 0, 430)

    teams = set_game_outcome(teams, week, -8,
                             'Seahawks', 13, 1, 133, 16, 31, 1, 0, 224,
                             'Cardinals', 27, 3, 159, 15, 27, 1, 0, 412)

    teams = set_game_outcome(teams, week, 2.5,
                             'Eagles', 17, 2, 313, 31, 40, 1, 0, 431,
                             'Cowboys', 9, 0, 257, 25, 44, 0, 0, 311)

    teams = set_game_outcome(teams, week, 7,
                             'Bears', 3, 0, 133, 18, 34, 0, 0, 234,
                             'Chiefs', 26, 3, 244, 23, 33, 2, 0, 350)

    teams = set_game_outcome(teams, week, -4,
                             'Vikings', 10, 1, 82, 16, 32, 1, 1, 139,
                             'Packers', 23, 2, 199, 26, 40, 0, 1, 383)
...
def week_17(teams, week_end_date, week=17):
    ...
    teams = set_game_outcome(teams,
                             ...
                             home_score=19,
                             home_touchdowns=1,
                             home_net_pass_yards=126,
                             home_pass_completions=12,
                             home_pass_attempts=21,
                             home_pass_tds=0,
                             ...
                             away_score=21,
                             away_touchdowns=1,
                             away_net_pass_yards=179,
                             away_pass_completions=26,
                             away_pass_attempts=37,
                             away_pass_tds=0,
                             away_interceptions_thrown=0,
                             away_total_yards=337)

    teams = set_game_outcome(teams, week, 1,
                             'Bills', 6, 0, 236, 21, 40, 0, 2, 309,
                             'Jets', 13, 1, 185, 23, 36, 1, 1, 271)

    teams = set_game_outcome(teams, week, -1,
                             'Buccaneers', 22, 2, 193, 13, 24, 2, 2, 329,
                             'Falcons', 28, 1, 286, 30, 56, 1, 0, 373)

    teams = set_game_outcome(teams, week, 14,
                             'Panthers', 10, 1, 288, 26, 49, 0, 2, 329,
                             'Saints', 42, 5, 264, 20, 31, 3, 0, 379)

    teams = set_game_outcome(teams, week, 2.5,
                             'Bengals', 33, 4, 182, 16, 28, 1, 1, 361,
                             'Browns', 23, 3, 239, 12, 27, 3, 3, 313)

    teams = set_game_outcome(teams, week, 13.5,
                             'Lions', 20, 2, 134, 13, 30, 1, 1, 305,
                             'Packers', 23, 2, 312, 27, 55, 2, 1, 432)

    teams = set_game_outcome(teams, week, -9.5,
                             'Chiefs', 31, 3, 174, 16, 25, 1, 1, 336,
                             'Chargers', 21, 3, 258, 31, 46, 2, 2, 366)

    teams = set_game_outcome(teams, week, -17,
                             'Patriots', 24, 3, 217, 16, 29, 2, 1, 352,
                             'Dolphins', 27, 2, 326, 29, 44, 1, 0, 389)

    teams = set_game_outcome(teams, week, 4,
                             'Giants', 17, 2, 275, 28, 47, 1, 1, 397,
                             'Eagles', 34, 4, 279, 23, 40, 1, 0, 400)

    teams = set_game_outcome(teams, week, 10,
                             'Texans', 14, 2, 192, 21, 36, 0, 1, 301,
                             'Titans', 35, 5, 222, 14, 21, 2, 0, 467)

    teams = set_game_outcome(teams, week, -12,
                             'Cowboys', 47, 5, 294, 23, 33, 4, 0, 517,
                             'Redskins', 16, 1, 183, 18, 37, 1, 1, 271)

    teams = set_game_outcome(teams, week, 5.5,
                             'Jaguars', 38, 3, 286, 27, 39, 3, 1, 353,
                             'Colts', 20, 2, 142, 12, 25, 0, 0, 275)

    teams = set_game_outcome(teams, week, 1.5,
                             'Ravens', 28, 1, 81, 11, 21, 0, 1, 304,
                             'Steelers', 10, 1, 77, 9, 25, 0, 0, 168)

    teams = set_game_outcome(teams, week, -4.5,
                             'Broncos', 16, 1, 168, 17, 28, 1, 0, 238,
                             'Raiders', 15, 1, 380, 29, 46, 1, 0, 477)

    teams = set_game_outcome(teams, week, -6.5,
                             'Rams', 31, 4, 311, 29, 45, 3, 0, 424,
                             'Cardinals', 24, 3, 322, 27, 43, 2, 2, 393)

    teams = set_game_outcome(teams, week, 3.5,
                             'Seahawks', 21, 3, 223, 25, 40, 2, 0, 348,
                             '49ers', 26, 3, 270, 18, 22, 0, 0, 398)
